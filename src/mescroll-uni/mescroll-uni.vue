    ...
    >
      <view :style="{ transform: translateY, transition: transition }">
        ...
        <view
          ...
          :class="{ 'empty-fixed': optEmpty.fixed }"
          :style="{ 'z-index': optEmpty.zIndex, top: optEmpty.top }"
        >
          <image
            v-if="optEmpty.icon"
            :class="
              optEmpty.icon.includes('empty') ? 'empty-img' : 'empty-icon'
            "
            :src="optEmpty.icon"
            mode="widthFix"
          />
          <view v-if="optEmpty.tip" class="empty-tip">{{ optEmpty.tip }}</view>
          <view v-if="optEmpty.btnText" class="empty-btn" @click="emptyClick">{{
            optEmpty.btnText
          }}</view>
        </view>
        ...
        <view v-if="mescroll.optUp.use" class="mescroll-upwarp">
          ...
          <!-- 无数据 -->
          <view v-if="!isDownLoading && isUpNoMore" class="upwarp-nodata">{{
  ...
  watch: {
    up: {
      handler(newName, oldName) {
        this.mescroll.optUp.empty.tip = newName.empty.tip;
        this.mescroll.optUp.empty.icon = newName.empty.icon;
      },
      deep: true, //为true，表示深度监听，这时候就能监测到up值变化
    },
  },
  computed: {
